...
【优先抓数据组合与轶事】数字组合（年份+次数+事件数，如"1868年两个月142起暴力/31谋杀/43枪击/55殴打"）和具体轶事（人名+情节）是最强指纹——独立写作者不会碰巧复现同一组数字；通史级表述（"1866年内战后成立"）是最弱指纹，不要单独作为指纹。
【引文去重】两条指纹的目标引文相同或互相包含时只保留一条——重复引文会被验证环节重复计命中。
${WORDING_RULES}
${JSON_DISCIPLINE}
输出 schema：
{"fingerprints":[{
...
要求：克制、具体、只引用给定证据；说明证据等级构成；如证据受限（部分取证/未独立复核）必须显式提及；【原创性纪律】署名完整、正式发表渠道（期刊/出版社/作协网站）只证明发布事实，不证明内容原创——禁止把它们当作支持「卫生」结论的证据引用；结尾自然带出"请依据材料自行判断"，不重复完整免责声明。
...
【关键】目标内容是播客时，源大概率也是播客（本法庭核心场景就是"播客依赖播客"）。topic 检索式必须包含：①至少一条带 podcast/episode 意图词 + 该主题最独特的叙事细节（具体事件/人物/作品名，不是通史词）的查询——用于找到讲同一主题的英文播客单集；②一条主题域通史式查询（用于兜底文章类源）。通史式查询（"history of X"）只能搜回百科与通史网站，永远找不到播客单集——不要只交这种。
【指纹式】优先用转录稿中的具体细节（人名/数字组合/作品名/轶事）构造，加引号短语。
${JSON_DISCIPLINE}
输出 schema：
{"queries":{
